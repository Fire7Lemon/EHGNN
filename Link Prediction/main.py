...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='PubMed', help='数据集：PubMed / DBLP（链接预测）')
    ...
    parser.add_argument('--wo_tweight', action='store_true', help='关闭目标类型权重')
    parser.add_argument('--r_neighbor', action='store_true', help='RW 后随机邻居')
    parser.add_argument('--K', type=int, default=10, help='Top-K 邻居数')
    parser.add_argument('--walk_num', type=int, default=100, help='每节点 RW 次数')
    ...
    parser.add_argument('--eps', type=float, default=1e-5, help='PPR 数值项（当前未用）')
    parser.add_argument('--alpha', type=float, default=0.5, help='自身与邻居融合系数 α')

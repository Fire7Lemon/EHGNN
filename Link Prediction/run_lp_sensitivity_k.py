"""Link Prediction：K 敏感性（对应论文 Fig.3）。

README LP 超参固定；仅扫描 --K。
LP main.py / main_yelp.py 默认 walk_num=100（与 README 未写明一致；不在本轮修改）。

输出 results/lp_sensitivity_k/；日志 <dataset>_K_<K>_seed_<seed>.log。"""
...
OUT_DIR = os.path.join(SCRIPT_DIR, 'results', 'lp_sensitivity_k')
...
CSV_FIELDS = [
    'dataset', 'K', 'seed', 'best_test_auc', 'best_test_ap', 'best_epoch',
...
def main():
    ap = argparse.ArgumentParser(description='LP K sensitivity (Fig.3)')
    ...
    lines = ['LP K sensitivity (Fig.3)', 'seeds: {}'.format(seeds), '']
    for ds in args.datasets:
        lines.append('=== {} ==='.format(ds))
        lines.append('K\tbest_AUC mean±std\tbest_AP mean±std')
        for k in sorted(set(args.ks)):
            au = [r['best_test_auc'] for r in rows if r['dataset'] == ds and r['K'] == k]
            ap = [r['best_test_ap'] for r in rows if r['dataset'] == ds and r['K'] == k]
            if not au:
                continue
            lines.append(
                '{}\t{:.6f} ± {:.6f}\t{:.6f} ± {:.6f}'.format(
                    k, np.mean(au), std_ddof(au), np.mean(ap), std_ddof(ap),
                ),
            )
        lines.append('')

"""Link Prediction：walk_num（随机游走次数 w）敏感性（对应论文 Table VIII）。

论文默认 walk_num = 40（常见论述）；本仓库 Link Prediction/main(.py|_yelp.py)
默认 --walk_num = 100（代码现状）。README LP 表未列出 walk_num。
本脚本 **显式传入** --walk_num 做扫描，**不修改**主入口默认值或数据。

输出 results/lp_sensitivity_walk_num/；日志 <dataset>_walk_<w>_seed_<seed>.log。"""
...
OUT_DIR = os.path.join(SCRIPT_DIR, 'results', 'lp_sensitivity_walk_num')

DEFAULT_SEEDS = [42, 3407, 2026, 6666, 8888]
DEFAULT_WALK_NUMS = [10, 25, 40, 55, 70]
...
CSV_FIELDS = [
    'dataset', 'walk_num', 'seed', 'best_test_auc', 'best_test_ap', 'best_epoch',
...
def main():
    ap = argparse.ArgumentParser(description='LP walk_num sensitivity (Table VIII)')
    ap.add_argument('--datasets', nargs='+', default=['PubMed', 'DBLP', 'Yelp'],
                    choices=['PubMed', 'DBLP', 'Yelp'])
    ap.add_argument('--walk_nums', nargs='+', type=int, default=DEFAULT_WALK_NUMS)
    ...
    for ds in args.datasets:
        ...
        for wn in args.walk_nums:
            for seed in seeds:
                log_name = '{}_walk_{}_seed_{}.log'.format(ds, wn, seed)
                log_path = os.path.join(OUT_DIR, log_name)
                if args.skip_existing:
                    m = try_log(log_path)
                    if m:
                        print('[SKIP]', log_name, flush=True)
                        rows.append({'dataset': ds, 'walk_num': wn, 'seed': seed, **m})
                        continue
                cmd = [sys.executable, exe] + base + ['--walk_num', str(wn), '--seed', str(seed)]
    ...
    lines = ['LP walk_num sensitivity (Table VIII)', 'seeds: {}'.format(seeds), '']
    lines.append(
        'Note: LP code default walk_num=100 if unset; this sweep overrides per run.',
    )
    lines.append('')
    for ds in args.datasets:
        lines.append('=== {} ==='.format(ds))
        lines.append('walk_num\tbest_AUC mean±std\tbest_AP mean±std')
        for wn in sorted(set(args.walk_nums)):
            au = [r['best_test_auc'] for r in rows if r['dataset'] == ds and r['walk_num'] == wn]
            ap = [r['best_test_ap'] for r in rows if r['dataset'] == ds and r['walk_num'] == wn]
            if not au:
                continue
            lines.append(
                '{}\t{:.6f} ± {:.6f}\t{:.6f} ± {:.6f}'.format(
                    wn, np.mean(au), std_ddof(au), np.mean(ap), std_ddof(ap),
                ),
            )
        lines.append('')

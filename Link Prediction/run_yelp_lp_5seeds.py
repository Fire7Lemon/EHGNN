"""Yelp Link Prediction：README LP 超参 × 多种子（默认 5 seeds）。

调用 **`main_yelp.py`**。输出：`results/yelp_lp_5seeds/`。
训练步日志里的 **`precision`** = AP（Average Precision）。"""
...
OUT_DIR = os.path.join(SCRIPT_DIR, 'results', 'yelp_lp_5seeds')
...
def parse_output(text, log_path_for_error):
    metrics = _parse_metrics_dict(text)
    if metrics is None:
        print('[FAIL] parse_output: missing Best/Final/Total lines', file=sys.stderr, flush=True)
...
def main():
    ap = argparse.ArgumentParser(description='Yelp Link Prediction — formal multi-seed (README LP)')
    ap.add_argument(
        '--seeds',
        type=int,
        nargs='+',
    ...
    rows = []
    base_cmd = [sys.executable, MAIN_YELP] + README_LP_CMD

    for seed in seeds:
        cmd = base_cmd + ['--seed', str(seed)]
        log_path = os.path.join(OUT_DIR, 'yelp_lp_seed_{}.log'.format(seed))
        ...
        if not reused:
            ...
            metrics = parse_output(merged, log_path)
    ...
    lines = [
        'Yelp Link Prediction — README LP hyperparameters',

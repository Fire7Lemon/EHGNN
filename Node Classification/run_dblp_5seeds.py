"""DBLP 节点分类多 seed 复现（reproduce-baseline）。

README.md「Usage」表格 Node Classification | DBLP 行（截至仓库当前版本）：
  alpha=0.7, K=20, lr=5e-4, dropout=0.5, hidden=512, layers=5, batch_size=3000

main.py 对 DBLP 不写 pubmed_nc_result.txt；本脚本从子进程或已有 .log 文本解析指标。
"""
...
OUT_DIR = os.path.join(RESULTS_ROOT, 'dblp_5seeds')
...
def parse_metrics_from_text(text):
    """从日志全文解析 DBLP main.py 文末指标；失败抛出 ValueError。"""
...
def _fail_subprocess(proc, cmd, log_path):
    ...
    print('[FAIL] --- stdout/stderr tail (last 80 lines) ---', file=sys.stderr, flush=True)
    print(_tail_lines(merged, 80) or '(empty)', file=sys.stderr, flush=True)
    sys.exit(1)
...
def parse_args():
    p = argparse.ArgumentParser(description='DBLP NC multi-seed runner (stdout/log metrics).')
    p.add_argument(
        '--seeds',
        type=int,
        nargs='+',
        default=None,
        metavar='SEED',
        help='要运行的随机种子列表；省略则使用默认 {}'.format(DEFAULT_SEEDS),
    )
    p.add_argument(
        '--skip_existing',
        action='store_true',
        help='若 dblp_seed_<seed>.log 已存在且可解析则跳过训练',
    )
    return p.parse_args()
...
def main():
    ...
    for seed in selected_seeds:
        log_path = os.path.join(OUT_DIR, 'dblp_seed_{}.log'.format(seed))
    ...
    lines = [
        'DBLP Node Classification — selected seeds (README hyperparameters)',

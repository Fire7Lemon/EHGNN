"""Node Classification：walk_num（随机游走次数 w）敏感性（对应论文 Table VIII）。

论文默认 walk_num = 40；Node Classification/main(.py|_yelp.py) 默认亦为 40。
本脚本显式传入 --walk_num 扫描列表；其余 README NC 超参按数据集固定。

输出 results/nc_sensitivity_walk_num/；日志 <dataset>_walk_<w>_seed_<seed>.log。"""
...
OUT_DIR = os.path.join(SCRIPT_DIR, 'results', 'nc_sensitivity_walk_num')
...
CSV_FIELDS = [
    'dataset', 'walk_num', 'seed', 'best_test_macro', 'best_test_micro', 'best_epoch',
...
def main():
    ap = argparse.ArgumentParser(description='NC walk_num sensitivity (Table VIII)')
    ...
    lines = ['NC walk_num sensitivity (Table VIII)', 'seeds: {}'.format(seeds), '']
    for ds in args.datasets:
        lines.append('=== {} ==='.format(ds))
        lines.append('walk_num\tbest_macro mean±std\tbest_micro mean±std')
        for wn in sorted(set(args.walk_nums)):
            ms = [r['best_test_macro'] for r in rows if r['dataset'] == ds and r['walk_num'] == wn]
            is_ = [r['best_test_micro'] for r in rows if r['dataset'] == ds and r['walk_num'] == wn]
            if not ms:
                continue
            lines.append(
                '{}\t{:.6f} ± {:.6f}\t{:.6f} ± {:.6f}'.format(
                    wn, np.mean(ms), std_ddof(ms), np.mean(is_), std_ddof(is_),
                ),
            )
        lines.append('')

...
**`docs/`** 为项目**中文说明文档目录**：本文件、`实验记录.md`、`论文复现报告.md`、`环境配置说明.md`、`GitHub提交检查清单.md`、`MAG240M运行说明.md`、**`TODO_metapath_experiments.md`**、**`TODO_hppr_strategy_study.md`** 等均位于此目录。仓库**根目录**主要保留 **`README.md`**（GitHub 首页）、**`environment.yml`**、**`requirements-freeze.txt`**、**`.gitignore`** 等入口与环境规格文件；数据集与脚本仍在 **`data/`**、**`scripts/`**、各任务子目录。
...
| **scripts** | MAG240M 校验与启动；**`server_run_core_reproduction.sh`** / **`server_run_extended_experiments.sh`**；**`run_server_resource_monitor.sh`**；**`collect_result_summaries.py`**。Fig.6 / Table IX / X 等规划见 **`TODO_metapath_experiments.md`**、**`TODO_hppr_strategy_study.md`**（与本文件同在 **`docs/`**）。 |

...
| **scripts** | MAG240M 校验与启动；**`server_run_core_reproduction.sh`** / **`server_run_extended_experiments.sh`**；**`run_server_resource_monitor.sh`**；**`collect_result_summaries.py`**。Fig.6 / Table IX / X 等规划见 **`元路径实验待办.md`**、**`HPPR策略研究待办.md`**（与本文件同在 **`docs/`**）。 |

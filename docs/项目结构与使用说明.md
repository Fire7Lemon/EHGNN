...
**`docs/`** 为项目**中文说明文档目录**：本文件、`实验记录.md`、`论文复现报告.md`、`环境配置说明.md`、`GitHub提交检查清单.md`、`MAG240M运行说明.md`、**`元路径实验待办.md`**、**`HPPR策略研究待办.md`** 等均位于此目录。仓库**根目录**主要保留 **`README.md`**（GitHub 首页）、**`environment.yml`**、**`requirements-freeze.txt`**、**`.gitignore`** 等入口与环境规格文件；数据集与脚本仍在 **`data/`**、**`scripts/`**、各任务子目录。

**Git clone 之后**：若仓库已跟踪 **`environment.yml`**、**`requirements-freeze.txt`**，它们会随 clone 到达本地，用于**规格与版本快照参考**；但不会自动创建 conda 环境或安装依赖。**已安装的 conda 环境与 `pip` 包**仍须在目标机器上按 **`环境配置说明.md`** 自行创建。**`data/`、`results/`、`logs/`** 通常也不在仓库中。

| 文件 | 作用 |
|------|------|
| **`environment.yml`** | conda 环境规格参考（非环境目录本体） |
| **`requirements-freeze.txt`** | 本机 pip freeze 版本快照参考 |
| **`环境配置说明.md`** | 解释服务器环境重建、CUDA 依赖、`environment.yml` / freeze 用法、`data`/`results` 同步问题 |

请先阅读 **`环境配置说明.md`**（服务器 / 本机环境与数据准备）。
...
├── environment.yml           # conda 环境规格参考（clone 后须 conda env create -f，见 docs/环境配置说明）
├── requirements-freeze.txt   # 本机 pip freeze 快照参考（GPU 包勿盲目 pip install -r）
├── docs/                     # 中文说明文档目录
│   ├── 项目结构与使用说明.md
│   ├── 实验记录.md
│   ├── 论文复现报告.md
...
│   └── HPPR策略研究待办.md
├── .gitignore
├── server_results/           # 可选：从服务器下载的结果归档（示例 core batch 见下 §10.1）
│   └── 2026-05-16_core_reproduction/
├── data/                     # 数据集（默认不提交 Git）
│   ├── PubMed/
│   ├── DBLP/
│   └── Yelp/
├── scripts/
│   ├── scripts目录说明.md          # scripts 下各 .py/.sh 用途与运行方式（权威索引）
│   ├── check_mag240m_data.py
│   ├── check_env.py
│   ├── collect_result_summaries.py   # → results/ALL_SUMMARIES_INDEX.md
│   ├── setup_env_linux_cu121.sh
│   ├── pack_env_linux.sh
│   ├── unpack_env_linux.sh
...
| **MAG240M** | OGB-LSC MAG240M 大规模节点分类；数据根目录语义见 `MAG240M运行说明.md`（`mag240m_kddcup2021/`）。 |
| **scripts** | MAG240M 校验与启动；**`server_run_core_reproduction.sh`** / **`server_run_extended_experiments.sh`**；**`run_server_resource_monitor.sh`**；**`collect_result_summaries.py`**；Linux 环境 **`setup_env_linux_cu121.sh`**、**`check_env.py`**、conda-pack **`pack_env_linux.sh`** / **`unpack_env_linux.sh`**。各文件用途与命令见 **`scripts/scripts目录说明.md`**。Fig.6 / Table IX / X 等规划见 **`元路径实验待办.md`**、**`HPPR策略研究待办.md`**（与本文件同在 **`docs/`**）。 |
| **data** | 论文三组数据（PubMed / DBLP / Yelp）及可选 MAG240M 父目录（本地放置）。 |
| **Node Classification/results** | PubMed：快照、5-seed、消融；DBLP：`dblp_5seeds/`；Yelp：**`yelp_5seeds/`**（**`run_yelp_5seeds.py`**）；历史 **`yelp_3seeds/`**。运行时产物；**正式 core 快照**可同时参见 **`server_results/`**（§10.1）。 |
| **`server_results/`** | **可选**：服务器下载归档（例如 **`2026-05-16_core_reproduction/`**）；内含 **`核心结果总结.md`** 与快照版 NC/LP **`summary`**（见 §10.1）。 |
...
| 单次 DBLP NC（README 超参） | 见 **`论文复现报告.md`** §5.2 |
...
`main.py` 默认 RW 后为 **频次 Top-K**；`--r_neighbor` 为随机邻居消融。**正式 core 数值**：见 **`server_results/2026-05-16_core_reproduction/`**（综述 **`核心结果总结.md`**）及 **`实验记录.md`** §2–§4、§0.4。
...
仓库根目录 **`../README.md`** 说明数据可置于 **`../data`**（相对于各子目录运行时即为仓库旁的 `data/`；当前常见做法是使用 **`Node Classification/../data`**，即仓库根下 `data/`）。

---

## 6. MAG240M：`--data_root` / `--path`

在 **`MAG240M/`** 下运行 `main.py`（见 `MAG240M运行说明.md`）：
...
| `yelp_3seeds/` | 历史 **`run_yelp_3seeds.py`**：**selected 3 seeds**。测试 F1 **跨 seed 完全相同**等症结见 **`实验记录.md`** §4。 |
...
### 10.1 服务器结果归档目录（`server_results/`，可选）

- 仓库可包含 **`server_results/`** 子目录，用于放置 **从服务器拷贝下来的完整实验快照**（通常 **不**等同于各子项目内的运行时 **`Node Classification/results/`**）。  
- **示例（core reproduction）**：**`server_results/2026-05-16_core_reproduction/`** — **core reproduction**（NC/LP 主干），**不包含**本目录内的消融或参数敏感性批量产物；综述见 **`核心结果总结.md`**；正式数值已同步至 **`实验记录.md`** §0.4、**`论文复现报告.md`** §6。  
- **Git**：若 `server_results/` 体积较大，可按团队约定加入 **`.gitignore`**；本文档仅说明其语义。

...
├── environment.yml           # conda 环境规格参考（clone 后须 conda env create -f，见环境配置说明）
├── requirements-freeze.txt   # 本机 pip freeze 快照参考（GPU 包勿盲目 pip install -r）
├── MAG240M运行说明.md         # MAG240M 服务器部署说明（扩展）
├── 环境配置说明.md      # Git clone 后：conda / PyTorch-DGL / data-results 说明（服务器环境）
├── .gitignore
├── data/                     # 数据集（默认不提交 Git）
│   ├── PubMed/
│   ├── DBLP/
